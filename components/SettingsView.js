...
export default function SettingsView({filteredRegions, setFilteredRegions}) {
  const showFiltered = (region) => ({region, isFiltered: !filteredRegions.includes(region)})
  const [isFilteredArray, setIsFilteredArray] = useState(regionsArray.map(showFiltered));

  const changeFilter = (target) => setFilteredRegions((prev) => {
  ...
  return (
    <SafeAreaView style={styles.safeContainer}>
      <View style={styles.container}>
        <FlatList
          data={isFilteredArray.map(({region, isFiltered}) => ({key: region, isFiltered}))}
          renderItem={({item}) => (
            <Pressable onPress={() => changeFilter(item.key)}>
              <Text style={item.isFiltered ? styles.pressableFilteredText : styles.pressableText}>{item.key}</Text>
            </Pressable>
          )}
        />
      </View>
